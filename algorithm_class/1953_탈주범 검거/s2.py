'''
dfs로 할때 주의할 점
dfs로 파고 내려갈때 어떤 지점에 도착했을 때, 
같은 지점에 동시에 또는 더 이른 시간에 도착한 이력이 있다면 
더 이상 탐색하지 않도록 하는 것입니다.
-> 왜냐하면 더 이른 시간에 도착한 경로가 더 멀리 갈 수 있기 때문에
그리고 더 느리게 도착한 녀석이 탐색을 더 먼저 할텐데 그러면 더 이른시간에
도착한 녀석의 길이 막히게 됨
'''
...
def dfs(r, c, time):
    ...
    # dictionary의 key로 사용할 현재 위치의 파이프값
    key = matrix[r][c]
